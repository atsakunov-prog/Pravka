...
3. **Внизу ровно шесть кнопок (Правка · Засечка · Дело · Спорт · Еда · Ещё), плавающих три: П · З · Д** (зелёная «Е» с 19.09.2026 выключена с завода, тумблер Еды возвращает её четвёртой). Связка и кольцо
   ...
   **диском** вокруг шестерёнки (`trigger/DiskController.kt`,
   ...
   слоёв — `core/DiskLook.kt` под тестами; кнопки и кружки веера выпуклые —
   `trigger/BubbleSkin.kt`, наследник `GradientDrawable` на том же поле
   контроллеров, нажатие гасит блик (`BubbleMotion.press`). На диске кнопка
   ...
   ключа — считает `DiskLook`, есть — слово за владельцем. Новая
