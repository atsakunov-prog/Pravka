...
- `slushalka/src/main/java/ru/zf/slushalka/`
  ...
  - `catalog/` — Флибуста по OPDS: `Opds.kt` (разбор Atom-ленты),
    ...
    `CatalogState.kt` (стопка лент, поиск, скачивание книги своей папкой
    `<Автор> - <Название>/` в корень библиотеки и перечитывание папки).
    Библиотека договорена жить в `Downloads/Books`; пикер открывается в Downloads.
