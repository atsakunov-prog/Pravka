...
3. **Внизу ровно шесть кнопок (Правка · Засечка · Дело · Спорт · Еда · Ещё), плавающих четыре: П · З · Д · инструменты** (зелёная «Е» с 19.09.2026 выключена с завода, тумблер Еды возвращает её пятой). Связка и кольцо
   говорят с кнопками через `trigger/RingButton.kt`; порядок — `chain()` в
   службе. Серые **инструменты** (`ToolsButtonController`): тап — веер
   быстрых настроек (модель чистки · микрофон · обновления · спрятать всё),
   долгое нажатие — экран настроек. Серая шестерёнка (0,72 кнопки) — не
   кнопка режима: в стопке над «П» (тап — веер), на диске в центре (тап —
   домой); двойной тап — всё в точку с «П». Плашки микрофона между «П» и
   «З» и трёх точек над «П» больше нет (18.09.2026). С 19.09.2026 кнопки по
   умолчанию стоят **диском** вокруг шестерёнки (`trigger/DiskController.kt`,
   `core/DiskGeometry.kt`): «П» сверху, «З» снизу, крутится за любую кнопку,
   переезжает за стекло (или за шестерёнку по долгому нажатию), у края виден
   до середины шестерёнки; выбор «Круг · Стопка» в Общих. Новая
