...
class TypographTest {
    ...
    @Test
    fun `после коротких слов пробел остаётся обычным`() {
        // Неразрывный пробел после предлога Android при выключке не тянет:
        // предлог слипается со словом, а соседние пробелы разъезжаются вдвое.
        // Рваная строка заметнее висячего предлога, поэтому клея здесь нет.
        assertEquals("и в лесу", typograph("и в лесу"))
    }
