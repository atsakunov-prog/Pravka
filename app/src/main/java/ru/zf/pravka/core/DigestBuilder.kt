...
class DigestBuilder(
        ...
        for (e in ribbon.filterNot { it.open }) {
            rows.add(
                Row(
                    ...
                    // Комментарий владельца к делу — тем же полем, после
                    // надиктовки: для читателя файла это его слова о том же
                    // отрезке, а отдельная колонка меняла бы обещанный формат.
                    note = listOf(e.raw.substringBefore("\nКБЖУ:").trim(), e.comment)
                        .filter { it.isNotBlank() }
                        .joinToString(" — комментарий: "),
                    id = "t${e.id}",
    ...
    private fun lifeCsvLegend(): String = """
        ...
        # дням и лежит в Notion, в базе «Телефон».
        #
        # КОЛОНКИ
        ...
        #   note          что было надиктовано; у дел таймшита после « — комментарий: »
        #                 идёт комментарий владельца к делу, если он его оставил.
        #                 У строк source=auto надиктовки нет вовсе, и пустое
        #                 поле здесь ничего не значит.
        #
    """.trimIndent() + "\n"
