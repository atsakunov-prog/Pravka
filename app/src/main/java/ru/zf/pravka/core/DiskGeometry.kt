...
object DiskGeometry {
    ...
    /**
     * На сколько кнопок за кромку стекла должен уйти палец, чтобы поворот
     * стал переездом. Полкнопки — это уже не «повёл по кольцу», а «вытянул
     * с диска».
     */
    const val TEAR_OFF = 0.5f

    /**
     * Палец, взявший кнопку, крутит диск — или уже ТЯНЕТ его? Владелец
     * (22.09.2026): «если я кручу эту штуку, то это такое короткое смахивание,
     * и я её кручу. А если я тяну, то явно тяну далеко и двигаю».
     *
     * «Далеко» меряется не путём пальца, а тем, ушёл ли он со стекла: поворот
     * ведут ПО кольцу, и палец, сколько бы ни прошёл, остаётся на диске; тянут
     * же прочь от него. Считать путь значило бы превратить в переезд длинный
     * поворот на пол-оборота, а считать направление в первый миг — решать
     * по одному миллиметру, где рука ещё не знает, чего хочет.
     *
     * [distance] — палец от центра КОЛЬЦА: у выдавленного диска кнопки стоят
     * вокруг него, и карманы стекла под ними — тоже. Кромка там, где кончается
     * карман: кольцо плюс [podRadius].
     */
    fun tornOff(distance: Float, ringRadius: Float, podRadius: Float, buttonSize: Int): Boolean =
        distance > ringRadius + podRadius + buttonSize * TEAR_OFF
