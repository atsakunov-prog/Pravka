 ...
 * всего) стоят по бокам от «лица» — направления внутрь экрана. Докованный к
 * правому краю диск смотрит влево: «П» вверху-слева, «З» внизу-слева, «Д» и
 * «Е» за краем. Так половина, которая видна, всегда та, что нужна.
 */
object DiskGeometry {
    ...
    /**
     * На сколько центр докованного диска отступает от края экрана: половина
     * шестерёнки и ещё полпросвета. Шестерёнка — ручка, за которую диск
     * переезжает, и тап по ней открывает веер; резать её краем нельзя, а
     * «наполовину видный» диск при таком отступе показывает чуть больше
     * половины — как раз две ближние кнопки целиком.
     */
    fun dockInset(gearSize: Int, gap: Int): Int = gearSize / 2 + gap / 2
     ...
     * [facing]: дома первая стоит на полшага против часовой от лица, дальше по
     * часовой через шаг. Одна кнопка — прямо на лице.
     */
    fun slotAngle(index: Int, count: Int, facing: Float, rotation: Float): Float {
        ...
        return norm(facing - step / 2f + index * step + rotation)
    }
