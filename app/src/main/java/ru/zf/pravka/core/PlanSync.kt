...
class PlanSync(
    ...
    /**
     * Прочитать страницу блока и вынуть из неё числа. Текст страницы кладём
     * целиком: он же уезжает в промпт разбора тренировок, где полезен прозой.
     */
    private suspend fun refreshRules(force: Boolean): Boolean {
        ...
        val parsed = claude.extractRules(page.text).getOrElse { e ->
            ...
            // сохраняем его, чтобы разбор тренировок им пользовался.
