...
object MoneyRules {
    ...
    val GENERIC: List<Rule> = listOf(
        ...
        Rule("перевод округлений", "roundup"),
        Rule("проценты на остаток|начисление процентов|выплата процентов", "inc_other", sign = 1),
        // Банкомат — всегда кошелёк ↔ банк: снял — кошелёк плюс, внёс — минус
        // (владелец, 25.09.2026: «пополнение через банкомат — это всегда то, что
        // я взял из наличного счёта»). Слово «банкомат» ловит и пуш, и выписку.
        Rule("внесение наличных|снятие в банкомате|выдача наличных|банкомат|пополнение наличными|взнос наличных|cash-in", "cash"),
