...
object PillLook {
    ...
    /** Поля пилюли от краёв экрана и зазор до пола, потолка и кнопок, dp. */
    const val SIDE_DP = 16
    const val GAP_DP = 10

    /**
     * Сверху пилюля выезжает из-под строки состояния, как уведомление: верх
     * окна стоит ровно по нижнему краю строки, и пилюля появляется из-за
     * этого края — путь [topTravelDp] (зазор и вся высота). Проскок у неё
     * ВНИЗ, поэтому запас окна — под пилюлей, [TOP_ROOM_DP]; окно при этом
     * не меняется ни на всплытии, ни на уходе.
     */
    const val TOP_ROOM_DP = 10

    fun topTravelDp(): Int = GAP_DP + HEIGHT_DP

    /**
     * Снизу без клавиатуры пилюля не опускается ниже этого над краем экрана,
     * dp (плюс [GAP_DP]). Владелец (26.09.2026): «если клавиатуры нет, то
     * плашка появляется очень низко на экране и выходит за границу экрана
     * вниз». У Samsung с жестами полоса навигации бывает нулевой, а углы
     * экрана круглые — пол «над навигацией» ложился в самый край. У Gemini
     * пилюля стоит примерно на 34 dp над краем — отсюда 24 + 10.
     */
    const val MIN_FLOOR_DP = 24
     ...
     * так вниз делает, как-то так выплыло и чуть-чуть — оп — назад». Сверху
     * то же зеркально: выехала, чуть проскочила вниз и вернулась. Это
    ...
    /**
     * Сколько держится итог — «что записал» (владелец, 26.09.2026: «плашка,
     * которая сама пропадает через, там, не знаю, секунд 5»). Раскрытый
     * тапом — дольше: его читают и правят.
     */
    const val RESULT_HOLD_MS = 5_000L
    const val RESULT_OPEN_HOLD_MS = 30_000L

    /**
     * Сколько держать итог, если вызывающий просил [requestedMs]. Короткие
     * служебные реплики (меньше двух секунд — «ещё раз, и пишу») живут своё;
     * записки о записанном — не меньше [RESULT_HOLD_MS]: прежние две секунды
     * Засечки не успевали прочитать.
     */
    fun resultHold(requestedMs: Long): Long =
        if (requestedMs < 2_000L) requestedMs else maxOf(requestedMs, RESULT_HOLD_MS)

    /** Итог-ошибка: пилюля в красных чернилах вместо цвета режима. */
    val ALERT = 0xFFD8342A.toInt()
