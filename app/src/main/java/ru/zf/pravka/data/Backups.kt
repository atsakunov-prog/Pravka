...
internal object Backups {
    ...
    // дневник еды, журнал силовых с сырыми надиктовками.
    ...
    private val STORES = listOf(
        ...
        // как и ленту.
        "food.json",
        // Журнал силовых. Самое незаменимое из всего списка: там лежат сырые
        // надиктовки, которые не удаляются никогда, — «гоблет четыре по десять
        // шестнадцать, последний тяжело» второй раз не скажется. И цепочка
        // зарядки, которой сорок дней, а восстановить её нечем.
        "strength.json",
        // Кэши сюда НЕ идут: sport.json отрастает из intervals.icu сам,
        // plan.json — из календаря и Notion.
    )
