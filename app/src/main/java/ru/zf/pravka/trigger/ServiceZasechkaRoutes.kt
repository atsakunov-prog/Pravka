...
// «не забыть» — в Todoist. Узнаёт команду `core/ZasechkaIntent.kt`; дороги —
...
/**
 * Разобрать сказанное по развилке. true — ушло не в ленту и разбор ленты не
 * нужен; false — обычная засечка (или режим, куда просится фраза, выключен
 * в профиле: тогда она честно ложится в ленту, а не пропадает).
 */
internal fun PravkaAccessibilityService.routeZasechka(text: String, spoken: Boolean): Boolean {
    val route = ZasechkaIntent.route(text)
    val word = when (route.kind) {
        ZasechkaIntent.Kind.ENTRY -> return false
        ZasechkaIntent.Kind.COMMENT -> "мысль"
        ZasechkaIntent.Kind.FOOD -> {
            if (!app.profileStore.has(Profile.Mode.FOOD)) {
                app.eventLog.add("засечка: «запиши еду», а Еда выключена в профиле — в ленту")
                return false
            }
            "еда"
        }
        ZasechkaIntent.Kind.TASKS -> {
            if (!app.profileStore.has(Profile.Mode.DELA)) {
                app.eventLog.add("засечка: «запиши дело», а Дела выключены в профиле — в ленту")
                return false
            }
            "дела"
        }
    }
    app.eventLog.add("засечка: развилка — $word: ${route.text}")
    ...
    when (route.kind) {
        ZasechkaIntent.Kind.COMMENT -> zasechkaThought(route.text, text, spoken)
