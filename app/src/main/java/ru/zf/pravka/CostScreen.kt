...
// Notion, эвалы, сверка паттернов при синке Notion. Список за «i» первой
// плашки — не обещание, а перечень тех самых вызовов.
...
internal fun CostScreen(app: PravkaApp) {
    ...
    Column(
        ...
        snapshot?.let { s ->
            // Подпись — о чём плашка, а не «Стоимость API»: так уже называется
            // шапка экрана (24.09.2026). «Что сюда входит» — за «i»: это была
            // отдельная плашка из одного пояснения.
            PaperCard(
                label = "сегодня и итоги",
                info = "Все обращения к Claude: чистка и ответы Правки, «сильнее», обучение и " +
        ...
        if (routes.isNotEmpty()) {
            PaperCard(
                label = "по дорогам, семь дней",
                info = "Считается с сборки 17.09: дни до неё — только в общей сумме. Дорогая строка — " +
                    "повод сменить модель или усилие в настройках, группа «Модели»: Fable стоит " +
                    "в два с половиной раза дороже Опуса 5.5 на входе и выходе. Батчи (ночной разбор, судья) показаны " +
                ...
                for (spend in routes) {
                    ...
                    val title = route?.let { "${it.mode} · ${it.title}" } ?: when (key) {
                        ...
                        // Третий проход разбора снят 18.09: ведро прошлых дней ещё живёт.
