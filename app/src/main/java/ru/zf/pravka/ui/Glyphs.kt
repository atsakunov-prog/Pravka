...
// Слушалке).
//
// Версия 3 (26.09.2026): почерк — выбор владельца. С завода «как у Gemini» —
// Material Symbols Rounded (`GlyphsGemini.kt`), и тогда в Material переходит
// ВЕСЬ набор, режимы тоже, — довод про чужой почерк рядом с пером выше не
// нарушается: почерк снова один. Здесь остаётся наш штрих — второе положение
// и запас для значка, которому у Google пары нет. Поэтому значки — геттеры:
