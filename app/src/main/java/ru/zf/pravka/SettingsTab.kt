...
private fun CommonSettings(app: PravkaApp, serviceEnabled: Boolean) {
    ...
    HintText(
        "Полминуты без касаний остаются две: «П» и «З» — они на своих местах " +
            ...
            "под «З» уголками; видно, что это стопка — из-под края торчат " +
            "рёбра, и тап по этому краю, чуть ниже «З», их возвращает. " +
            "Посреди записи или разбора не складываются никогда."
    )
