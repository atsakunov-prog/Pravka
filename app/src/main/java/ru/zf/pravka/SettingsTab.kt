...
private fun CommonSettings(app: PravkaApp, serviceEnabled: Boolean) {
    ...
    HintText(
        "Круг: «П» сверху, «З» снизу, дальше «Д» и серые инструменты — вокруг " +
            "шестерёнки на стекле. Повёл кнопку по кругу — крутится весь диск и " +
            "щёлкает по четвертям, махнул сильнее — провернётся дальше; тап и долгое " +
            "нажатие по кнопке — как были. Тянешь за стекло (края круга, промежутки " +
            "между кнопками) — диск переезжает; у края экрана прячется до середины " +
            "шестерёнки: «П» и «З» внутри, «Д» и инструменты за краем, докрутить их — " +
            "пальцем. Инструменты: тап — веер быстрых настроек, долгое нажатие — этот " +
            "экран. Шестерёнка: тап — диск домой, двойной тап — всё в точку «П» (тап " +
            "по точке возвращает), долгое нажатие и тянуть — тоже переезд. Стопка: " +
            "прежний столбик с ручкой-галочкой, инструменты четвёртые. Зелёная «Е» " +
            "выключена — вернуть её пятой можно тумблером в настройках Еды."
    )
    ...
    HintText(
        "На диске — полминуты без касаний возвращают его домой: «П» и «З» " +
            ...
            "(«Д» и инструменты уезжают в «З»; ниже про «Е» — то же самое, если она включена) " +
