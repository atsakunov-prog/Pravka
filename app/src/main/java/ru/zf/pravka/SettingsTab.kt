...
private fun CommonSettings(app: PravkaApp, serviceEnabled: Boolean) {
    ...
    HintText(
        "Полминуты без касаний остаются две: «П» и «З» — они на своих местах " +
            ...
            "схлопываются в «З», а под стопкой остаётся стрелка-галочка: «⌄» " +
            "выпускает их обратно, «⌃» убирает. Стрелкой можно свернуть и " +
            "разложить когда угодно, не дожидаясь полуминуты. Посреди записи " +
            "или разбора сами не складываются никогда."
    )
