...
8. ТОЛЬКО ФОН → "parallel": «параллельно слушаю Акунина», «фоном подкаст»,
   «под это дело музыка». Текущее дело НЕ закрывается и минут не теряет.
   Поля те же, что у "new".
   ФОН К УЖЕ ЗАПИСАННОМУ ДЕЛУ — тот же "parallel" плюс "entry": «ой, пока
   был в туалете, смотрел ютуб», «во время обеда говорил с мамой», «на
   встрече параллельно разбирал почту». Найди это дело в списке «Сегодня»
   и верни его номер — границы параллели возьмутся у него, придумывать
   время не надо. Дела в списке нет, но названо время — верни
   "start_time"/"end_time" вместо номера.
   ВАЖНО: «пока был в X, делал Y» — это НЕ новая запись и НЕ вставка X.
   X уже в ленте и трогать его нельзя; записывается Y параллелью к X.
...
parallel: {"action": "parallel", "entry": 0, "title": "...", "category": "...", "client": "...", "start_offset_min": 0, "start_time": "", "end_time": ""}
          ("entry" — номер дела из списка «Сегодня», поверх которого это шло; 0 = не названо)
...
- ПАРАЛЛЕЛЬНОСТЬ. Какими словами он говорит, что два дела шли разом
  («заодно», «под это дело», «пока… , то…»), и не потерялось ли второе.
