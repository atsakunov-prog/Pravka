...
ПРАВИЛА ПОЛЕЙ:
- "title": КОНКРЕТНОЕ название дела, 2–6 слов, именной группой, с большой
  буквы, без глаголов: «Медкомиссия с Серёжей», «Поездка на дачу за
  детьми», «Приготовление еды». НИКОГДА не называй дело именем категории:
  title «Быт», «Отдых», «Потери», «Семья» ЗАПРЕЩЁН — категория лежит в
  своём поле, а title должен отвечать «что именно происходило», с людьми
  и местами, которые владелец назвал. Через неделю он должен по title
  вспомнить этот час. Если такое же дело уже есть в списках «Сегодня» или
  «Прошлые дни» — назови ТОЧНО так же, буква в букву: одинаковые дела
  называются одинаково, иначе неделя не складывается.
