...
class ModeGlowTest {
    ...
    fun `оттенок не уходит ни в одном тоне свечения`() {
        for (a in accents) {
            ...
            for (t in listOf(ModeGlow.tone(a), ModeGlow.light(a), ModeGlow.deep(a))) {
    ...
    fun `тёмные чернила кнопок зажигаются, яркие остаются собой`() {
        ...
        // «П» и так яркая — тон совпадает с кнопкой.
        assertEquals(0xFFEA580C.toInt(), ModeGlow.tone(0xFFEA580C.toInt()))
    }
